...
(function(j) {
	...
	var l = function(u) {
		...
		for (y = 0; y < u.length; y++) {
			for (x = 0; x < u[y].length; x++) {
				var s = u[y][x], v = s.rowspan || 1, t = s.colspan || 1, z = j
						.inArray(false, B[y]);
				if (t === 1) {
					s.fieldIndex = z;
	...
	var a = function() {
		if (k === null) {
			var t = j("<p/>").addClass("fixed-table-scroll-inner"), u = j(
					"<div/>").addClass("fixed-table-scroll-outer"), s, r;
			u.append(t);
			j("body").append(u);
			s = t[0].offsetWidth;
	...
	var f = function(s, r, w) {
		var x = Object.getOwnPropertyNames(s), u = Object
				.getOwnPropertyNames(r), v = "";
		if (w) {
	...
	var h = function() {
		if (!Object.keys) {
			Object.keys = (function() {
				var t = Object.prototype.hasOwnProperty, u = !({
					toString : null
				}).propertyIsEnumerable("toString"), s = [ "toString",
						"toLocaleString", "valueOf", "hasOwnProperty",
						"isPrototypeOf", "propertyIsEnumerable", "constructor" ], r = s.length;
				return function(x) {
					if (typeof x !== "object"
		...
		if (this.options.locale) {
			...
			if (j.fn.bootstrapTable.locales[this.options.locale]) {
				j.extend(this.options,
						j.fn.bootstrapTable.locales[this.options.locale])
			} else {
				if (j.fn.bootstrapTable.locales[r.join("-")]) {
					j.extend(this.options, j.fn.bootstrapTable.locales[r
							.join("-")])
				} else {
					if (j.fn.bootstrapTable.locales[r[0]]) {
						j.extend(this.options,
								j.fn.bootstrapTable.locales[r[0]])
					}
				}
			}
		}
	...
	e.prototype.initHeader = function() {
							...
							j
									.each(
											...
											function(B, A) {
												var F = "", C = "", E = "", w = "", D = m(
														' class="%s"',
														A["class"]), z = t.options.sortOrder
														|| A.order, y = "px", x = A.width;
												...
												if (typeof A.fieldIndex !== "undefined") {
													...
													t.header.styles[A.fieldIndex] = E
															+ w;
												...
												s
														.push(m(
																...
																t.options.sortable
																		&& A.sortable ? "sortable both"
																		: ""));
												F = A.title;
												if (A.checkbox) {
													if (!t.options.singleSelect
		...
		if (!this.options.showHeader || this.options.cardView) {
			...
			this.getCaret();
			j(window).on("resize.bootstrap-table",
					j.proxy(this.resetWidth, this))
		}
	...
	e.prototype.initSort = function() {
		var u = this, t = this.options.sortName, r = this.options.sortOrder === "desc" ? -1
				: 1, s = j.inArray(this.options.sortName, this.header.fields);
		...
		if (s !== -1) {
			...
			this.data
					.sort(function(w, v) {
						...
						var y = o(w, t, u.options.escape), z = o(v, t,
								u.options.escape), x = q(u.header,
								u.header.sorters[s], [ y, z ]);
						if (x !== undefined) {
							return r * x
						}
	...
	e.prototype.onSort = function(r) {
		var t = r.type === "keypress" ? j(r.currentTarget) : j(r.currentTarget)
				.parent(), s = this.$header.find("th").eq(t.index());
		...
		this.options.sortName = t.data("sortName") ? t.data("sortName") : t
				.data("field");
	...
	e.prototype.initToolbar = function() {
		...
		if (typeof this.options.toolbar === "string"
			...
			j(
					m('<div class="bs-bars pull-%s"></div>',
							this.options.toolbarAlign)).appendTo(this.$toolbar)
					.append(j(this.options.toolbar))
		}
		t = [ m('<div class="columns columns-%s btn-group pull-%s">',
				this.options.buttonsAlign, this.options.buttonsAlign) ];
		if (typeof this.options.icons === "string") {
			this.options.icons = q(null, this.options.icons)
		}
		if (this.options.showSearch) {
			t.push(m('<button class="btn'
					+ m(" btn-%s", this.options.buttonsClass)
					+ m(" btn-%s", this.options.iconSize)
					+ '" type="button" name="showSearch" title="%s">',
					this.options.formatSearch()), m('<i class="%s %s"></i>',
					this.options.iconsPrefix, this.options.icons.search),
					"</button>")
		}
		if (this.options.showPaginationSwitch) {
			t.push(m('<button class="btn'
					...
					this.options.formatPaginationSwitch()), m(
		...
		if (this.options.showRefresh) {
			t.push(m('<button class="btn'
					...
					this.options.formatRefresh()), m('<i class="%s %s"></i>',
					this.options.iconsPrefix, this.options.icons.refresh),
					"</button>")
		}
		if (this.options.showToggle) {
			t.push(m('<button class="btn'
					...
					.formatToggle()), m('<i class="%s %s"></i>',
					this.options.iconsPrefix, this.options.icons.toggle),
					"</button>")
		}
		if (this.options.showColumns) {
			t.push(m('<div class="keep-open btn-group" title="%s">',
					this.options.formatColumns()),
					'<button type="button" class="btn'
							+ m(" btn-%s", this.options.buttonsClass)
							+ m(" btn-%s", this.options.iconSize)
							+ ' dropdown-toggle" data-toggle="dropdown">', m(
							'<i class="%s %s"></i>', this.options.iconsPrefix,
		...
		if (this.options.showPaginationSwitch) {
			this.$toolbar.find('button[name="paginationSwitch"]').off("click")
					.on("click", j.proxy(this.togglePagination, this))
		}
		if (this.options.showRefresh) {
			this.$toolbar.find('button[name="refresh"]').off("click").on(
					"click", j.proxy(this.refresh, this))
		}
		if (this.options.showToggle) {
		...
		if (this.options.showSearch) {
			this.$toolbar.find('button[name="showSearch"]').off("click").on(
					"click", function() {
		...
		if (this.options.showColumns) {
			...
			s.find("input").off("click")
					.on(
							"click",
							function() {
								var x = j(this);
								u.toggleColumn(j(this).val(),
										x.prop("checked"), false);
								u.trigger("column-switch", j(this)
										.data("field"), x.prop("checked"))
							})
		}
		if (this.options.search) {
			...
			t.push(
					'<div class="pull-' + this.options.searchAlign
							+ ' search">', m('<input class="form-control'
							+ m(" input-%s", this.options.iconSize)
	...
	e.prototype.initSearch = function() {
		...
		if (this.options.sidePagination !== "server") {
			...
			var r = this.searchText
					&& (this.options.escape ? p(this.searchText)
							: this.searchText).toLowerCase();
			var u = j.isEmptyObject(this.filterColumns) ? null
					: this.filterColumns;
			this.data = u ? j.grep(this.options.data, function(w, v) {
				for ( var s in u) {
					if (j.isArray(u[s]) && j.inArray(w[s], u[s]) === -1
							|| w[s] !== u[s]) {
						return false
					}
				}
			...
			this.data = r ? j.grep(this.data, function(A, x) {
				for (var v = 0; v < t.header.fields.length; v++) {
					...
					var w = j.isNumeric(t.header.fields[v]) ? parseInt(
							t.header.fields[v], 10) : t.header.fields[v];
	...
	e.prototype.initPagination = function() {
		...
		var v = this, x = [], r = false, A, z, s, w, G, I, E, y, u, J = this
				.getData(), t = this.options.pageList;
		if (this.options.sidePagination !== "server") {
		...
		if (this.options.totalRows) {
			if (this.options.pageSize === this.options.formatAllRows()) {
				...
				if (this.options.pageSize === this.options.totalRows) {
					var H = typeof this.options.pageList === "string" ? this.options.pageList
							.replace("[", "").replace("]", "")
		...
		if (!this.options.onlyInfoPagination) {
			...
			var F = [
					...
					r ? this.options.formatAllRows() : this.options.pageSize,
					"</span>", ' <span class="caret"></span>', "</button>",
					'<ul class="dropdown-menu" role="menu">' ];
			if (typeof this.options.pageList === "string") {
				var D = this.options.pageList.replace("[", "").replace("]", "")
						.replace(/ /g, "").split(",");
				t = [];
				j.each(D, function(K, L) {
					t.push(L.toUpperCase() === v.options.formatAllRows()
							.toUpperCase() ? v.options.formatAllRows() : +L)
				})
			}
			j.each(t, function(K, L) {
				if (!v.options.smartDisplay || K === 0
						|| t[K - 1] <= v.options.totalRows) {
					var M;
					if (r) {
					...
					F.push(m('<li%s><a href="javascript:void(0)">%s</a></li>',
							M, L))
				}
			});
			F.push("</ul></span>");
			...
			x
					.push(
							...
							'<div class="pull-' + this.options.paginationHAlign
									+ ' pagination">',
							'<ul class="pagination'
									+ m(" pagination-%s", this.options.iconSize)
									+ '">',
			...
			for (A = z; A <= s; A++) {
				x.push('<li class="page-number'
						+ (A === this.options.pageNumber ? " active" : "")
						+ '">', '<a href="javascript:void(0)">', A, "</a>",
						"</li>")
			}
			if (this.totalPages >= 8) {
				if (this.options.pageNumber <= (this.totalPages - 4)) {
					x.push('<li class="page-last-separator disabled">',
			...
			x.push('<li class="page-next"><a href="javascript:void(0)">'
		...
		if (this.options.showPageGo) {
			var v = this, C = this.$pagination.find("ul.pagination"), B = C
					.find("li.pageGo");
			if (!B.length) {
				B = j(
						[
								...
								'<button class="btn'
										...
										+ m(" btn-%s", this.options.iconSize)
										+ '" title="'
										+ this.options.formatPageGo() + '" '
										+ ' type="button">'
										+ this.options.formatPageGo(),
								"</button>", "</li>" ].join("")).appendTo(C);
	...
	e.prototype.onPageListChange = function(r) {
		...
		this.options.pageSize = s.text().toUpperCase() === this.options
	...
	e.prototype.initBody = function(x) {
		...
		for (var w = this.pageFrom - 1; w < this.pageTo; w++) {
			...
			y.push("<tr", m(" %s", A.join(" ")), m(' id="%s"',
					j.isArray(C) ? undefined : C._id), m(' class="%s"',
					r.classes || (j.isArray(C) ? undefined : C._class)), m(
					' data-index="%s"', w), m(' data-uniqueid="%s"',
					C[this.options.uniqueId]), m("%s", t), ">");
			if (this.options.cardView) {
				y.push(m('<td colspan="%s"><div class="card-views">',
						this.header.fields.length))
			}
			if (!this.options.cardView && this.options.detailView) {
				y.push("<td>", '<a class="detail-icon" href="javascript:">', m(
						'<i class="%s %s"></i>', this.options.iconsPrefix,
			...
			j
					.each(
							this.header.fields,
							function(I, L) {
								var P = "", M = o(C, L, z.options.escape), K = "", E = {}, Q = "", J = z.header.classes[I], G = "", O = "", R = "", H = "", F = z.columns[I];
								...
								E = q(z.header, z.header.cellStyles[I], [ M, C,
										w, L ], E);
								if (E.classes) {
								...
								if (E.css) {
									...
									r = m('style="%s"', D.concat(
											z.header.styles[I]).join("; "))
								}
								...
								if (F.checkbox || F.radio) {
									...
									P = [
											m(
													z.options.cardView ? '<div class="card-view %s">'
											...
											"<input"
													...
													+ m(
															' disabled="%s"',
															!F.checkboxEnabled
																	|| (M && M.disabled) ? "disabled"
																	: undefined)
													+ " />",
											z.header.formatters[I]
													&& typeof M === "string" ? M
													: "",
											z.options.cardView ? "</div>"
													: "</td>" ].join("");
									C[z.header.stateField] = M === true
											|| (M && M.checked)
								} else {
									M = typeof M === "undefined" || M === null ? z.options.undefinedText
											: M;
									P = z.options.cardView ? [
											'<div class="card-view">',
											z.options.showHeader ? m(
													'<span class="title" %s>%s</span>',
													r, c(z.columns, "field",
															"title", L))
													: "",
											m('<span class="value">%s</span>',
													M), "</div>" ].join("")
											: [
													m(
															"<td%s %s %s %s %s %s %s>",
															Q, J, r, G, O, R, H),
													M, "</td>" ].join("");
									if (z.options.cardView
											&& z.options.smartDisplay
											&& M === "") {
		...
		if (!y.length) {
			y.push('<tr class="no-records-found">', m(
		...
		this.$body
				...
				.on(
						"click dblclick",
						function(J) {
							var D = j(this), F = D.parent(), M = z.data[F
									.data("index")], H = D[0].cellIndex, G = z
									.getVisibleFields(), K = G[z.options.detailView
									&& !z.options.cardView ? H - 1 : H], E = z.columns[i(
									z.columns, K)], L = o(M, K,
									z.options.escape);
							if (D.find(".detail-icon").length) {
								return
							}
							z.trigger(J.type === "click" ? "click-cell"
									: "dbl-click-cell", K, L, M, D);
							z.trigger(J.type === "click" ? "click-row"
									: "dbl-click-row", M, F, K);
							if (J.type === "click" && z.options.clickToSelect
									&& E.clickToSelect) {
								var I = F.find(m('[name="%s"]',
										z.options.selectItemName));
								if (I.length) {
									I[0].click()
								}
		...
		this.$body
				...
				.on(
						"click",
						function() {
							var H = j(this), G = H.parent().parent(), E = G
									.data("index"), I = v[E];
							if (G.next().is("tr.detail-view")) {
								H.find("i").attr(
										"class",
										m("%s %s", z.options.iconsPrefix,
								...
								H.find("i").attr(
										"class",
										m("%s %s", z.options.iconsPrefix,
												z.options.icons.detailClose));
								G
										.after(m(
												'<tr class="detail-view"><td colspan="%s"></td></tr>',
												G.find("td").length));
								var D = G.next().find("td");
								var F = q(z.options, z.options.detailFormatter,
										[ E, I, D ], "");
								if (D.length === 1) {
									D.append(F)
								}
		...
		this.$selectItem = this.$body.find(m('[name="%s"]',
		...
		this.$selectItem
				...
				.on(
						...
						function(E) {
							...
							var F = j(this), D = F.prop("checked"), G = z.data[F
									.data("index")];
							if (z.options.maintainSelected
									&& j(this).is(":radio")) {
							...
							if (z.options.singleSelect) {
								z.$selectItem
										...
										.each(
												function() {
													z.data[j(this)
															.data("index")][z.header.stateField] = false
												});
								z.$selectItem.filter(":checked").not(this)
										.prop("checked", false)
							}
							z.updateSelected();
		...
		j
				.each(
						...
						function(G, F) {
							...
							var H = z.header.fields[G], D = j.inArray(H, z
									.getVisibleFields());
	...
	e.prototype.initServer = function(r, w, s) {
		...
		if (this.options.pagination) {
			x.pageSize = this.options.pageSize === this.options.formatAllRows() ? this.options.totalRows
					: this.options.pageSize;
		...
		if (this.options.queryParamsType === "limit") {
			...
			if (this.options.pagination) {
				x.offset = this.options.pageSize === this.options
						.formatAllRows() ? 0 : this.options.pageSize
						* (this.options.pageNumber - 1);
				x.limit = this.options.pageSize === this.options
		...
		t = j.extend({}, q(null, this.options.ajaxOptions), {
			...
			data : this.options.contentType === "application/json"
					&& this.options.method === "post" ? JSON.stringify(v) : v,
				...
				.each(
						...
						function(s, t) {
							j(t)
									...
									.addClass(
											(j(t).data("field") === r.options.sortName || j(
													t).data("sortName") === r.options.sortName) ? r.options.sortOrder
													: "both")
						})
	};
	e.prototype.updateSelected = function() {
		var r = this.$selectItem.filter(":enabled").length
		...
		if (s.length > 0) {
			r = this.$selectItem.filter(":enabled").length
					&& this.$selectItem.filter(":enabled").length === s.filter(
							":enabled").filter(":checked").length
		}
		this.$selectAll.add(this.$selectAll_).prop("checked", r);
		this.$selectItem.each(function() {
			j(this).closest("tr")[j(this).prop("checked") ? "addClass"
	...
	e.prototype.updateRows = function() {
		var r = this;
		this.$selectItem.each(function() {
			r.data[j(this).data("index")][r.header.stateField] = j(this).prop(
					"checked")
		})
	};
	...
	e.prototype.resetHeader = function() {
		...
		this.timeoutId_ = setTimeout(j.proxy(this.fitHeader, this), this.$el
				.is(":hidden") ? 100 : 0)
	};
	e.prototype.fitHeader = function() {
		...
		r = u.scrollWidth > u.clientWidth
				&& u.scrollHeight > u.clientHeight + this.$header.outerHeight() ? a()
				: 0;
		...
		this.$header.find("th[data-field]").each(
				function(B) {
					t.$header_.find(
							m('th[data-field="%s"]', j(this).data("field")))
							.data(j(this).data())
				});
	...
	e.prototype.resetFooter = function() {
		...
		this.timeoutFooter_ = setTimeout(j.proxy(this.fitFooter, this),
	...
	e.prototype.toggleColumn = function(r, s, u) {
		...
		this.initBody();
		if (this.options.showColumns) {
			var t = this.$toolbar.find(".keep-open input").prop("disabled",
					false);
			if (u) {
	...
	e.prototype.toggleRow = function(r, t, s) {
		...
		this.$body.find(typeof r !== "undefined" ? m('tr[data-index="%s"]', r)
				: m('tr[data-uniqueid="%s"]', t))[s ? "show" : "hide"]()
	};
	...
	e.prototype.resetView = function(u) {
		...
		if (this.options.height) {
			var t = d(this.$toolbar), v = d(this.$pagination), r = this.options.height
					- t - v;
	...
	e.prototype.getData = function(r) {
		return (this.searchText || !j.isEmptyObject(this.filterColumns) || !j
				.isEmptyObject(this.filterColumnsPartial)) ? (r ? this.data
				.slice(this.pageFrom - 1, this.pageTo) : this.data)
				: (r ? this.options.data.slice(this.pageFrom - 1, this.pageTo)
						: this.options.data)
	};
	...
	e.prototype.mergeCells = function(z) {
		var y = z.index, t = j.inArray(z.field, this.getVisibleFields()), u = z.rowspan || 1, s = z.colspan || 1, w, v, x = this.$body
				.find(">tr"), r;
	...
	e.prototype.updateCell = function(r) {
		if (!r.hasOwnProperty("index") || !r.hasOwnProperty("field")
	...
	e.prototype.check_ = function(t, r) {
		var s = this.$selectItem.filter(m('[data-index="%s"]', r)).prop(
				"checked", t);
	...
	e.prototype.checkBy_ = function(s, u) {
		...
		j.each(this.options.data, function(v, x) {
			...
			if (j.inArray(x[u.field], u.values) !== -1) {
				var w = r.$selectItem.filter(":enabled").filter(
						m('[data-index="%s"]', v)).prop("checked", s);
	...
	e.prototype.togglePagination = function() {
		...
		if (this.options.pagination) {
			r.attr("class", this.options.iconsPrefix + " "
	...
	e.prototype.toggleAllColumns = function(r) {
		...
		this.initBody();
		if (this.options.showColumns) {
			var s = this.$toolbar.find(".keep-open input").prop("disabled",
					false);
			...
			if (v) {
				try {
					s = setInterval(function() {
						u = x.$body.find("tr.detail-view").last().find(
								".detail-icon");
						if (u.length > 0) {
							u.click()
						} else {
	...
	var n = [ "getOptions", "getSelections", "getAllSelections", "getData",
			"load", "append", "prepend", "remove", "removeAll", "insertRow",
			"updateRow", "updateCell", "updateByUniqueId", "removeByUniqueId",
			"getRowByUniqueId", "showRow", "hideRow", "getRowsHidden",
			"mergeCells", "checkAll", "uncheckAll", "checkInvert", "check",
			"uncheck", "checkBy", "uncheckBy", "refresh", "resetView",
			"resetWidth", "destroy", "showLoading", "hideLoading",
	...
	j.fn.bootstrapTable = function(s) {
		...
		this.each(function() {
			var w = j(this), v = w.data("bootstrap.table"), u = j.extend({},
